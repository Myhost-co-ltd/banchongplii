        ...
        <form method="GET" class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-3">
            <div class="flex flex-wrap items-center gap-3">
                <div class="flex items-center gap-2 bg-white border border-gray-200 shadow-sm rounded-2xl px-4 py-2.5">
                    ...
                    <select id="course" name="course" class="rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm"
                ...
                <div class="flex items-center gap-2 bg-white border border-gray-200 shadow-sm rounded-2xl px-4 py-2.5">
                    ...
                    <select id="grade" name="grade" class="rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm"
            ...
            <div class="flex items-center gap-2 bg-white border border-gray-200 shadow-sm rounded-2xl px-4 py-2.5">
                ...
                <input id="q" name="q" value="{{ $search }}" placeholder="ชื่อหลักสูตร / ชื่อครู / ชั้น"
                       class="rounded-xl border-gray-300 focus:border-blue-500 focus:ring-blue-500 text-sm w-56"
                       onkeydown="if(event.key==='Enter'){this.form.submit();}" />
                @if ($search !== '')
                    <button type="button" class="text-xs text-gray-500 hover:text-gray-700" onclick="document.getElementById('q').value=''; this.form.submit();">ล้าง</button>
                @endif
            </div>
        </form>
    </div>
                        ...
                        @if (! empty($course->rooms))
                            <div class="flex flex-wrap gap-2">
                                @foreach ($course->rooms as $room)
                                    <span class="px-3 py-1 text-xs bg-blue-50 text-blue-700 rounded-full border border-blue-100">{{ $room }}</span>
                                @endforeach
                            </div>
                        @endif
