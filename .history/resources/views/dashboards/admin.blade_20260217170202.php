...
<div class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-6">
    ...
    <!--จำนวนห้อง -->
    <div class="relative p-6 bg-purple-100 border border-purple-200 rounded-2xl shadow-sm">
        ...
        <button type="button"
                ...
                class="mt-3 text-sm text-purple-700 font-semibold hover:underline"
        ...
        <div id="roomDropdownPanel" class="hidden absolute left-0 top-full mt-3 w-[22rem] max-w-[85vw] rounded-2xl border border-purple-200 bg-white p-4 shadow-2xl z-30">
    ...
    <div class="p-6 bg-sky-100 border border-sky-200 rounded-2xl shadow-sm">
        <h3 class="text-gray-600 mb-1" data-i18n-th="หลักสูตรเสร็จแล้ว" data-i18n-en="Teachers with finished courses">หลักสูตรเสร็จแล้ว</h3>
    ...
    <div class="p-6 bg-amber-100 border border-amber-200 rounded-2xl shadow-sm">
        <h3 class="text-gray-600 mb-1" data-i18n-th="หลักสูตรยังไม่เสร็จ" data-i18n-en="Teachers with unfinished courses">หลักสูตรยังไม่เสร็จ</h3>
...
<div class="mt-10 bg-white rounded-3xl shadow p-8 border border-gray-100">
    <div class="flex flex-col lg:flex-row items-start gap-8 lg:gap-12">
        <div class="flex-1 space-y-4">
            <div>
                <h2 class="text-2xl font-semibold text-gray-900">สถานะหลักสูตรข</h2>
...
<script>
    ...
    function closeRoomDropdown() {
        const panel = document.getElementById('roomDropdownPanel');
    ...
    function toggleRoomDropdown(event) {
        ...
        if (roomSelect && roomSelect.value) {
            ...
            if (bodyEl) bodyEl.innerHTML = '<p class="py-3 text-sm text-gray-500 text-center">Please select a classroom</p>';
        }
    }
    ...
    function renderRoomStudents(room) {
        ...
        if (!room) {
            ...
            bodyEl.innerHTML = '<p class="py-3 text-sm text-gray-500 text-center">Please select a classroom</p>';
            return;
        }

        summaryEl.textContent = `Room ${room} • ${list.length} students`;
        bodyEl.innerHTML = '';

        if (!list.length) {
            bodyEl.innerHTML = '<p class="py-3 text-sm text-gray-500 text-center">No students in this classroom</p>';
            return;
        }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const roomPanel = document.getElementById('roomDropdownPanel');
        const roomToggle = document.getElementById('roomDropdownToggle');

        document.addEventListener('click', (event) => {
            if (!roomPanel || roomPanel.classList.contains('hidden')) return;
            const clickedInsidePanel = roomPanel.contains(event.target);
            const clickedToggle = roomToggle?.contains(event.target);
