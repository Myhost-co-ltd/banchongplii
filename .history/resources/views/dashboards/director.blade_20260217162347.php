...
<div class="space-y-10 overflow-y-auto pr-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
        ...
        <div id="student-room-section" class="relative p-6 bg-gradient-to-br from-purple-50 to-purple-200 border border-purple-300 rounded-2xl shadow w-full text-left transition hover:-translate-y-0.5 hover:shadow-lg">
            ...
            <div id="roomDropdownPanel"
                ...
                <select id="roomSelect"
                    ...
                    @foreach(($studentsByRoom ?? collect()) as $room => $students)
                        <option value="{{ $room }}">{{ $room }} ({{ number_format(collect($students)->count()) }} students)</option>
                    @endforeach
                </select>
         ...
         <button type="button"
                ...
                class="p-6 bg-gradient-to-br from-green-50 to-green-200 border border-green-300 rounded-2xl shadow w-full text-left transition hover:-translate-y-0.5 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2">
            <div class="flex items-start justify-between gap-3">
                <div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const roomStudentsData = @json(
            collect($studentsByRoom ?? collect())->map(function ($students) {
