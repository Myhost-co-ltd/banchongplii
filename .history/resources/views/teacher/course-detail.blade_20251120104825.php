    ...
    @if($course)
        ...
        <section class="bg-white rounded-3xl shadow-md p-6 border border-gray-100">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('teacher.courses.edit', $course) }}"
                   class="text-sm text-blue-600 hover:underline">จัดการข้อมูล</a>
            </div>
            <div class="space-y-3">
                ...
                @empty
                    <p class="text-gray-500 text-sm text-center">ยังไม่มีการบันทึกจำนวนชั่วโมงที่สอน</p>
                @endforelse
            </div>
        </section>
        ...
        <section class="bg-white rounded-3xl shadow-md p-6 border border-gray-100">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('teacher.courses.edit', $course) }}"
                   class="text-sm text-blue-600 hover:underline">จัดการบทเรียน</a>
            </div>
            <div class="space-y-3">
                ...
                @empty
                    <p class="text-gray-500 text-sm text-center">ยังไม่มีหัวข้อบทเรียนที่บันทึกไว้</p>
                @endforelse
            </div>
        </section>
        ...
        <section class="bg-white rounded-3xl shadow-md p-6 border border-gray-100 mb-10">
            <div class="flex justify-between items-center mb-4">
                ...
                <a href="{{ route('teacher.courses.edit', $course) }}"
                   class="text-sm text-blue-600 hover:underline">จัดการการบ้าน</a>
            </div>
            <div class="space-y-3">
                    ...
                    <div class="border border-gray-100 rounded-2xl p-4">
                        ...
                        <p class="text-sm text-gray-600 mt-1">
                            ...
                            @if(!empty($assignment['due_date']))
                                ...
                                กำหนดส่ง: {{ \Carbon\Carbon::parse($assignment['due_date'])->locale('th')->isoFormat('D MMM YYYY') }}
                            @endif
                        </p>
                ...
                @empty
                    <p class="text-gray-500 text-sm text-center">ยังไม่มีการมอบหมายงานสำหรับหลักสูตรนี้</p>
                @endforelse
            </div>
        </section>
    @endif
   ...
   @unless($course)
    <div class="bg-white rounded-3xl shadow-md p-10 border border-gray-100 text-center">
        ...
        <p class="text-gray-600 mb-6 max-w-3xl mx-auto">
            กรุณาสร้างหลักสูตรใหม่จากหน้าสร้างหลักสูตร หรือเลือกหลักสูตรจากรายการที่มีเพื่อดูรายละเอียด
        </p>
        <a href="{{ route('teacher.course-create') }}"
...
@endunless   {{-- ✅ เปลี่ยนเป็น @endunless --}}
</div>
